...
namespace App\ConnectionPools\Parser;

use function App\ConnectionPools\Common\startsWith;
use App\ConnectionPools\Parser\Util\ParseUtil;
        ...
        if (strlen($stmt) > ++$offset) {
            switch ($stmt[$offset]) {
                ...
                case 't':
                    if (strlen($stmt) > ++$offset) {
                        //支持一下语句
                        //  /*!App\ConnectionPools: sql=SELECT * FROM test where id=99 */set @pin=1;
//                    call p_test(@pin,@pout);
//                    select @pout;
                        if (startsWith($stmt, "/*!App\ConnectionPools:") || startsWith($stmt, "/*#App\ConnectionPools:") || startsWith($stmt, "/*App\ConnectionPools:")) {
