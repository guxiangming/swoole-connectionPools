<?php 
/*
 * @Description: 进程服务的管理监听模块
 * @Author: czm
 * @Date: 2019-04-28 12:36:31
 * @LastEditTime: 2019-05-09 10:54:31
 */
...
use \App\ConnectionPools\Handler\Frontend\FrontendAuthenticator;
use \App\ConnectionPools\Handler\Frontend\FrontendConnection;
use \App\ConnectionPools\MysqlPacket\MySqlPacketDecoder;
use \App\ConnectionPools\MysqlPacket\BinaryPacket;
use \App\ConnectionPools\MysqlPacket\OkPacket;
use \App\ConnectionPools\MysqlPacket\AuthPacket;
use \App\ConnectionPools\MysqlPacket\Util\RandomUtil;
use \App\ConnectionPools\MysqlPacket\Util\ErrorCode;
use \App\ConnectionPools\MysqlPacket\MySQLPacket;
use function \App\ConnectionPools\Common\packageSplit;
use function \App\ConnectionPools\Common\array_copy;
use function \App\ConnectionPools\Common\getString;
use function \App\ConnectionPools\Common\getBytes;
use function \App\ConnectionPools\Common\getMysqlPackSize;
use function \App\ConnectionPools\Common\initConfig;
use function \App\ConnectionPools\Common\getPackageLength;
use \App\ConnectionPools\Route\RouteService;
use \App\ConnectionPools\Parser\ServerParse;
use \App\ConnectionPools\MysqlPool\MySQLPool;
use \App\ConnectionPools\MysqlPool\MySQLException;

class ConnectionServer extends BasePoolsServer
{
    public $source;
    public $mysqlClient;
    protected $dbConfig;
    ...
    public function onReceive(\swoole_server $server, int $fd, int $reactor_id, string $data)
    {
        self::go(function () use ($server, $fd, $reactor_id, $data) {
            if (!isset($this->source[$fd]->auth)) {
                throw new ConnectionPoolsException('Must be connected before sending data!');
            }
    ...
    public function onWorkerStart(\swoole_server $server, int $worker_id)
    {
        self::go(function () use ($server, $worker_id) {
            if ($worker_id >= CONFIG['server']['swoole']['worker_num']) {
                Log::fileWrite("ConnectionServer-onWorkerStart['task process']");
            }else{
                Log::fileWrite("ConnectionServer-onWorkerStart['worker process']");
            }
            try {
                ...
                Log::fileWrite("ConnectionServer-onWorkerStart['mysql']".$exception);
                $server ->shutdown();
                return;
            } catch (ConnectionPoolsException $exception) {
                Log::fileWrite("ConnectionServer-onWorkerStart['system']".$exception);
                $server ->shutdown();
                return;
            }
            if ($worker_id === (CONFIG['server']['swoole']['worker_num'] - 1)) {
                ...
                Log::fileWrite("ConnectionServer-onWorkerStart['Worker started!']");
        ...
        foreach ($this->dbConfig as $key => $value) {
            ...
            while ($value['startConns']) {
                ...
                switch (explode(DB_DELIMITER, $key)[0]) {
                    case 'read':
                        $mysql->query('/*CP test sql*/select sleep(0.1)');
                        break;
                    case 'write':
                        $mysql->query('/*CP test sql*/set autocommit=1');
                        break;
                }
                $clients[] = $mysql;
                $value['startConns']--;
    ...
    {
        $message = 'CP@access denied for user \'' . $this->source[$fd]->user . '\'@\'' .
    ...
    private function existsDBKey(\swoole_server $server, int $fd, string $model, string $key)
    {
        //如果没有写库则抛出异常
        if (!isset($this->dbConfig[$key])) {
            $message = 'CP@Database config ' . ($this->source[$fd]->database ?: '') . ' ' . $model .
                ' is not exists!';
                    ...
                    switch ($hintArr['db_type']) {
                        case 'read':
                            if ($queryType == ServerParse::DELETE || $queryType == ServerParse::INSERT ||
                                ...
                                Log::fileWrite("system should not use hint 'db_type' to route 'delete', 'insert', 'replace', 'update', 'ddl' to a slave db.");
                                echo "system should not use hint 'db_type' to route 'delete', 'insert', 'replace', 'update', 'ddl' to a slave db.".PHP_EOL;
                            } else {
                        ...
                        default:
                            $this->connectReadState[$fd] = false;
                            Log::fileWrite("system use hint 'db_type' value is not found.");
                            echo "use hint 'db_type' value is not found.".PHP_EOL;
                            break;
                    }
